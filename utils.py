...
def cal_gross_profit_margin(total_op_in,op_costs,R_and_D_exp,business_tariff_and_annex):
   #毛利率 =（营业收入 - 营业成本 - 研发费用 - 营业税金及附加）/ 营业收入
   if total_op_in == 0:
      return 0
   return round(float(total_op_in - op_costs - R_and_D_exp - business_tariff_and_annex) / total_op_in * 100,1)
...
def cal_operating_margin_of_safety(total_op_in,op_costs,R_and_D_exp,business_tariff_and_annex,op_profit):
   #经营安全边际率 = 营业利益率 / 营业毛利率
   gross_profit_margin = cal_gross_profit_margin(total_op_in,op_costs,R_and_D_exp,business_tariff_and_annex)
